...
sys_read(read_args_t *arg)
{
	read_args_t temp;
	void *buf;
	int ret = copy_from_user(&temp, arg, sizeof(temp));
	if(ret < 0)
	{
	...
	buf = page_alloc();
	int ret1 = do_read(temp.fd, buf, temp.nbytes);
	if(ret1 < 0)
	{
		curthr->kt_errno = -ret1;
		return -1;
	}
	int ret2 = copy_to_user(arg->buf, buf, temp.nbytes);
	if(ret2 < 0)
	{
		curthr->kt_errno = -ret2;
		return -1;
	}
	page_free(buf);
	return ret1;
...
sys_write(write_args_t *arg)
{
	...
	buf = page_alloc();
	int ret1 = copy_from_user(buf, arg->buf, arg->nbytes);
	if(ret1 < 0)
	{
		curthr->kt_errno = -ret1;
		return -1;
	}
	int ret2 = do_write(temp.fd, buf, temp.nbytes);
	if(ret2 < 0)
	{
		curthr->kt_errno = -ret2;
		return -1;
...
sys_getdents(getdents_args_t *arg)
{
  ...
  if (copy_from_user(&temp, arg, sizeof(temp)) < 0) {
    curthr->kt_errno = EFAULT;
    return -1;
  }
  ...
  uint32_t bytes = 0;
  int ret;
  while(bytes < temp.count)
    {
      if((ret = do_getdent(temp.fd, dir)) < 0)
	{
	  curthr->kt_errno = -ret;
	  return -1;
	}
	...
	break;
      bytes += ret;
      copy_to_user( arg->dirp, dir, sizeof(dirent_t));
    }
  ...
  dbg(DBG_CORE,"Read %u bytes in sys_getdents\n",bytes);
  return bytes;
}
...
static int sys_munmap(munmap_args_t *args)
{
  ...
  dbg(DBG_CORE," kargs addr = %x, kargs.len = %x\n",(uint32_t)kargs.addr,(uint32_t)kargs.len);
...
static void *sys_mmap(mmap_args_t *arg)
{
  ...
  if (err < 0) {
    curthr->kt_errno = -err;
    dbg(DBG_CORE,"error = %d\n",err);
    return MAP_FAILED;
  }
  /*		copy_to_user( kaddr,const void * uaddr,size_t nbytes)*/
  return ret;
}
