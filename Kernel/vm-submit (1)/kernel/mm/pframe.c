...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
		...
		if(pageoutd_needed()) pageoutd_wakeup();
...
pframe_pin(pframe_t *pf)
{
	...
	dbg(DBG_PRINT,"(GRADING3A 1.a) Pframe is not free\n");

			KASSERT(pf->pf_pincount >= 0);
			dbg(DBG_PRINT,"(GRADING3A 1.a) Pframe pincount is not less than 0\n");


	if(pframe_is_free(pf)) return;
...
pframe_unpin(pframe_t *pf)
{
	...
	dbg(DBG_PRINT,"(GRADING3A 1.b) Pframe is not free\n");
			...
			KASSERT(pf->pf_pincount > 0);
			dbg(DBG_PRINT,"(GRADING3A 1.b) Pframe pincount is greater than 0\n");


	if(pframe_is_free(pf) || (!pframe_is_pinned(pf))) return;
