...
{
	dbg(DBG_PRINT, "Entering special file read......\n");
	KASSERT(file);
	dbg(DBG_PRINT, "(GRADING2A 1.a) File is valid..\n");
	KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
	dbg(DBG_PRINT, "(GRADING2A 1.a) It is either a block device or a character device....\n");
	if(S_ISCHR(file->vn_mode))
	{
        KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
        dbg(DBG_PRINT, "(GRADING2A 1.a) Character device with valid read operation\n");
		bytedev_t *bytedevptr = file->vn_cdev;
		int nbr = bytedevptr->cd_ops->read(bytedevptr, offset, buf, count);     /* returns number of bytes read */
		dbg(DBG_PRINT, "Exiting special file read......\n");
		return nbr;
	}
...
{
	KASSERT(file);
	dbg(DBG_PRINT, "(GRADING2A 1.b) File is valid\n");
	KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
	dbg(DBG_PRINT, "(GRADING2A 1.b) It is either a block device or a character device....\n");
	if(S_ISCHR(file->vn_mode))
	{
		KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
		dbg(DBG_PRINT, "(GRADING2A 1.b) Character device with valid read operation\n");
		bytedev_t *bytedevptr = file->vn_cdev;

		int nbw = bytedevptr->cd_ops->write(bytedevptr, offset, buf, count);     /* returns number of bytes read */
		/*dbg(DBG_PRINT, "Hi6\n");*/
		return nbw;
	}
	else if(S_ISBLK(file->vn_mode))
...
{
	KASSERT(file);
	dbg(DBG_PRINT, "GRADING3A 5.a - The vnode of special file exists");
	KASSERT(S_ISCHR(file->vn_mode) && "because these ops only assigned if vnode represents a special file");
	dbg(DBG_PRINT, "GRADING3A 5.a - vnode represents a special file (character device)");
    KASSERT((file->vn_cdev) && "because open shouldn\'t have let us arrive here if vn_cdev was NULL");
    dbg(DBG_PRINT, "GRADING3A 5.a - the special file has a valid reference to the character device");
    KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->mmap);
    dbg(DBG_PRINT, "GRADING3A 5.a -> The memory map operation of the device exists");
	int mmap_ret = 0;
	if(file->vn_cdev != NULL)
	{
		mmap_ret = file->vn_cdev->cd_ops->mmap(file, vma, ret);
	}
	else
		return -ENOTSUP;
	return mmap_ret;
...
{
	KASSERT(file);
	dbg(DBG_PRINT, "GRADING3A 5.b - The vnode of special file exists");
	KASSERT(S_ISCHR(file->vn_mode) && "because these ops only assigned if vnode represents a special file");
	dbg(DBG_PRINT, "GRADING3A 5.b - vnode represents a special file (character device)");
    KASSERT((file->vn_cdev) && "because open shouldn\'t have let us arrive here if vn_cdev was NULL");
    dbg(DBG_PRINT, "GRADING3A 5.b - the special file has a valid reference to the character device");
	KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->fillpage);
    dbg(DBG_PRINT, "GRADING3A 5.b -> The fillpage operation of the device exists");

    int fillpage_ret = 0;
    if(S_ISCHR(file->vn_mode))
    {
    	fillpage_ret = file->vn_cdev->cd_ops->fillpage(file, offset, pagebuf);
    }
    else
    	return -ENOTSUP;
	return fillpage_ret;
...
{
	KASSERT(file);
	dbg(DBG_PRINT, "GRADING3A 5.c - The vnode of special file exists");
	KASSERT(S_ISCHR(file->vn_mode) && "because these ops only assigned if vnode represents a special file");
	dbg(DBG_PRINT, "GRADING3A 5.c - vnode represents a special file (character device)");
    KASSERT((file->vn_cdev) && "because open shouldn\'t have let us arrive here if vn_cdev was NULL");
    dbg(DBG_PRINT, "GRADING3A 5.c - the special file has a valid reference to the character device");
	KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->dirtypage);
    dbg(DBG_PRINT, "GRADING3A 5.c -> The dirtypage operation of the device exists");

    int dirtypage_ret = 0;
    if(S_ISCHR(file->vn_mode))
    {
    	dirtypage_ret = file->vn_cdev->cd_ops->dirtypage(file, offset);
    }
    else
    	return -ENOTSUP;
	return dirtypage_ret;
...
{
	KASSERT(file);
	dbg(DBG_PRINT, "GRADING3A 5.d - The vnode of special file exists");
	KASSERT(S_ISCHR(file->vn_mode) && "because these ops only assigned if vnode represents a special file");
	dbg(DBG_PRINT, "GRADING3A 5.d - vnode represents a special file (character device)");
    KASSERT((file->vn_cdev) && "because open shouldn\'t have let us arrive here if vn_cdev was NULL");
    dbg(DBG_PRINT, "GRADING3A 5.d - the special file has a valid reference to the character device");
	KASSERT(file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->cleanpage);
    dbg(DBG_PRINT, "GRADING3A 5.d -> The cleanpage operation of the device exists");

    int cleanpage_ret = 0;
    if(S_ISCHR(file->vn_mode))
    {
    	cleanpage_ret = file->vn_cdev->cd_ops->cleanpage(file, offset, pagebuf);
    }
    else
    	return -ENOTSUP;
	return cleanpage_ret;
